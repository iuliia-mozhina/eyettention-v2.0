...
class Eyettention(nn.Module):
	def __init__(self, cf):
		...
		encoder_config = BertConfig.from_pretrained(self.cf["model_pretrained"]) # you can specify the pre-trained model in the config
		...
		self.attn = nn.Linear(self.hidden_size, self.hidden_size+1) # +1 account for the word length feature
		...
		self.decoder_duration_cell1 = nn.LSTMCell(768 + 2 + 51, self.hidden_size)  # TODO: not sure about the input size for duration prediction
	...
	def decode_duration(self, dec_emb_in, location_output):
	...
	#	print("dec_emb_in", dec_emb_in, dec_emb_in.shape)  # [39, 256, 770]
	#	print("location_output", location_output, location_output.shape) # torch.Size([39, 256, 51])
		...
		for j in range(dur_emb_in.shape[0]):
			...
			duration_result = self.decoder_duration(hx_dur)
	...
	def decode(self, sp_emd, sn_mask, sp_pos, enc_out, sp_fix_dur, sp_landing_pos, word_ids_sp):
		...
		location_output, atten_weights_batch = self.decode_location(sp_emd, sn_mask, sp_pos, enc_out, sp_fix_dur, sp_landing_pos,
																	word_ids_sp)

		# Phase 2: Decode fixation duration using learned representations from location prediction
		dec_emb_in = self.prepare_input_for_duration_prediction(sp_emd, sp_pos, sp_landing_pos, sp_fix_dur, word_ids_sp)
		duration_output = self.decode_duration(dec_emb_in, location_output)

		return location_output, duration_output, atten_weights_batch

	def prepare_input_for_duration_prediction(self, sp_emd, sp_pos, sp_landing_pos, sp_fix_dur, word_ids_sp):
		...
		# Concatenate additional gaze features (landing pos and fix_dur)
		if sp_landing_pos is not None:
			dec_emb_in = torch.cat((dec_emb_in, sp_landing_pos.permute(1, 0)[:-1, :, None]), dim=2)
	...
	def scanpath_generation(self, sn_emd,
		...
		# prepare the input for the duration prediction
		result_size = (dec_in.shape[0], 51)  # 51 is dec_o_dim
		zeros_placeholder = torch.zeros(result_size).to(sn_emd.device)
		dur_emb_in = torch.cat([dec_in, zeros_placeholder], dim=1)
		# dec_in is [256, 770]
		# zeros_placeholder is [256, 51]

		#generate fixation one by one in an autoregressive way
		output = []
		density_prediction = []

		batch_size = dur_emb_in.shape[0]  # 256
		hx_dur = torch.zeros(batch_size, self.hidden_size).to(dur_emb_in.device)
		cx_dur = torch.zeros(batch_size, self.hidden_size).to(dur_emb_in.device)
		...
		for p in range(max_pred_len-1):
			# print("dec_in", dec_in, dec_in.shape)
		...
		#	print("dur_emb_in", dur_emb_in, dur_emb_in.shape)
			...
			hx_dur, cx_dur = self.decoder_duration_cell2(self.dropout_LSTM_duration(hx_dur), (hx_dur, cx_dur))
			hx_dur, cx_dur = self.decoder_duration_cell3(self.dropout_LSTM_duration(hx_dur), (hx_dur, cx_dur))
			result_dur = self.decoder_duration(hx_dur)
			duration_output.append(result_dur)
			...
			dec_in = torch.cat((dec_emb_in, torch.zeros(dec_emb_in.shape[0],2).to(sn_emd.device)), dim=1)
			# prepare the input for the duration prediction
			# dec_emb_in [39, 256, 770] = dec_in [256, 770]
			# location_output" [39, 256, 51] = result [256, 51]
			dur_emb_in = torch.cat([dec_in, result], dim=1)
